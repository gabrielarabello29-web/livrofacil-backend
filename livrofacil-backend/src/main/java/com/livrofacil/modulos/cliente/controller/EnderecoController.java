...
public class EnderecoController {
    ...
    @PostMapping
    public ResponseEntity<EnderecoResponse> criar(@PathVariable UUID clienteId, @Valid @RequestBody EnderecoRequest request) {
        return ResponseEntity.status(201).body(enderecoUseCase.criar(clienteId, request));
    }

    // Listar enderecos do cliente
    @GetMapping
    public ResponseEntity<List<EnderecoResponse>> listar(@PathVariable UUID clienteId) {
        return ResponseEntity.ok(enderecoUseCase.listar(clienteId));
    }

    // Buscar endereco por ID
    @GetMapping("/{enderecoId}")
    public ResponseEntity<EnderecoResponse> buscarPorId(@PathVariable UUID clienteId, @PathVariable Long enderecoId) {
        return ResponseEntity.ok(enderecoUseCase.buscarPorId(clienteId, enderecoId));
    }

    // Editar endereco do cliente
    @PutMapping("/{enderecoId}")
    public ResponseEntity<EnderecoResponse> atualizar(@PathVariable UUID clienteId, @PathVariable Long enderecoId,
    ...
    public ResponseEntity<Void> excluir(@PathVariable UUID clienteId, @PathVariable Long enderecoId) {
